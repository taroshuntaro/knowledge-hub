...
## ドキュメントの追従（画面 / API / 構成図）

[`docs/screens.md`](./docs/screens.md)（画面 / 機能一覧）・[`docs/api.md`](./docs/api.md)（API 一覧）・[`docs/assets/`](./docs/assets/) の構成図はプロジェクト把握用のリファレンス。**実装を変えたら同じ変更セットで追従させる。**
...
- **構成図（`docs/assets/architecture.svg` / `docs/assets/architecture-aws.svg`、README に掲載）:** 外部依存（PostgreSQL / S3 互換ストレージ / SMTP / OIDC IdP）やサーバーの層構成を増減した、あるいは `config.ts` の環境変数を追加 / 削除したとき。AWS リソースは [AWS Architecture Icons](https://aws.amazon.com/architecture/icons/)（公式アイコン）で表記する。
