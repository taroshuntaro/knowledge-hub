...
- **認証** — 事前許可制ログイン（管理者が事前登録したユーザーのみログイン可能）、セッション、パスワードリセット、ユーザー管理（有効化 / 無効化・ロール）。OIDC 汎用ログイン（PKCE・事前作成行の初回 SSO クレーム・email 自動リンク・ドメイン制限）にも対応し、パスワード認証との共存 / OIDC 専用運用を切り替え可能。
- **アカウントライフサイクル** — ユーザーの一括登録（CSV / 個別追加）と一括無効化（CSV / 複数選択）。パスワード認証の初回ログインは期限付きの共通登録コード + 本人によるパスワード設定（メール基盤なしで完結）。誤登録の削除・「未ログインに戻す」による復旧にも対応。
...
docs/            設計書・実装計画・リファレンス（画面一覧 / API 一覧）
```

全体像を素早く掴むには次のリファレンスが早い。

- [`docs/screens.md`](./docs/screens.md) — 画面 / 機能一覧（各画面の権限・主な機能）
- [`docs/api.md`](./docs/api.md) — API 一覧（メソッド・パス・認可・概要）
