...
> **⚠️ WIP** — 社内向けブログ型ナレッジ共有サービス。開発中（Phase 2a まで完了）です。

小規模チーム（〜100人程度）向けの、ブログ中心のナレッジ共有サービス。記事は **Markdown を正**として管理し、2 階層カテゴリ + フリータグで分類、全体フィードを主役に据えます。オンプレ / AWS の両対応（差分は環境変数のみ）を前提に設計しています。
