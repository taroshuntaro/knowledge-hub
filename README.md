...
> **⚠️ WIP** — チームのためのブログ型ナレッジ共有サービス。開発中（Phase 4b まで完了）です。
...
- [ ] **UI/UX 刷新** — サイドバー型アプリシェル、記事サムネイル（16:9 ヒーロー画像）、インディゴ配色、情報設計・一貫性の底上げ（S1〜S4）
  - [x] **S1** — デザイントークン（インディゴ・AA 再検証）+ アプリシェル（サイドバー / モバイルドロワー = Radix Dialog・カテゴリ 2 階層・アカウントメニュー）
  - [ ] **S2** — 記事ヒーロー画像（スキーマ + エディタ設定 + 一覧 API 拡張）
  - [ ] **S3** — 統一記事カード + フィード / ピックアップ + 各一覧画面
  - [ ] **S4** — 記事詳細 / エディタ / 検索統合 / 認証・設定・管理 + 一貫性パス
