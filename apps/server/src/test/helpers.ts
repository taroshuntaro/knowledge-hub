...
export async function resetDb(db: Db) {
  await db.execute(
    sql`truncate table article_tags, article_revisions, articles, tags, categories, uploads, users, sessions, password_reset_tokens, registration_codes, departments, positions cascade`,
  );
}
